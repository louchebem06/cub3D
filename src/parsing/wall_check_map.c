								...
								int x, int y)
{
	if (d.up >= 0 && d.up < d.y_map)
		if (!ft_isset_tab(map[d.up][x], "1")
			&& !ft_isset_tab(map[d.up][x], "0NSWE"))
			return (0);
	if (d.down >= 0 && d.down < d.y_map)
		if (!ft_isset_tab(map[d.down][x], "1")
			&& !ft_isset_tab(map[d.down][x], "0NSWE"))
			return (0);
	if (d.left >= 0 && d.left < d.x_map)
		if (!ft_isset_tab(map[y][d.left], "1")
			&& !ft_isset_tab(map[y][d.left], "0NSWE"))
			return (0);
	if (d.right >= 0 && d.right < d.x_map)
		if (!ft_isset_tab(map[y][d.right], "1")
			&& !ft_isset_tab(map[y][d.right], "0NSWE"))
			return (0);
	if ((d.up < 0 || d.up >= d.y_map)
...
static int	check_space(t_direction_parsing_wall dir, char **map, int x, int y)
{
	if (dir.up >= 0 && dir.up < dir.y_map)
		if (!ft_isset_tab(map[dir.up][x], "1")
			&& map[dir.up][x] != ' ')
			return (0);
	if (dir.down >= 0 && dir.down < dir.y_map)
		if (!ft_isset_tab(map[dir.down][x], "1")
			&& map[dir.down][x] != ' ')
			return (0);
	if (dir.left >= 0 && dir.left < dir.x_map)
		if (!ft_isset_tab(map[y][dir.left], "1")
			&& map[y][dir.left] != ' ')
			return (0);
	if (dir.right >= 0 && dir.right < dir.x_map)
		if (!ft_isset_tab(map[y][dir.right], "1")
			&& map[y][dir.right] != ' ')
			return (0);
	return (1);
}
