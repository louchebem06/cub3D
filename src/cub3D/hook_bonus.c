...
/*   Updated: 2021/10/13 13:40:46 by bledda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	move_mouse(int button, int x, int y, t_cub *cub)
{
	(void)cub;
	printf("button:%d\nx:%d\ny:%d\n", button, x, y);
	return (0);
...
void	hook(t_cub *cub)
{
	...
	//mlx_hook(cub->win.win, 6, 1L << 7, move_mouse, cub);
	mlx_mouse_hook(cub->win.win, move_mouse, cub);
